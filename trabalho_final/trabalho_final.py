...
# Filtro
imgf = cv2.GaussianBlur(img, (7, 7), 10)
cv2.imshow("Aplicado o filtro gaussiano", imgf)
#imgf = cv2.medianBlur(img, 5)
#cv2.imshow("Aplicado o filtro da mediana", imgf)
cv2.waitKey()

rel, img_r_lim = cv2.threshold(img, 145, 255, cv2.THRESH_BINARY)
cv2.imshow("Imagem ruidosa limiarizada", img_r_lim)
...
rel, orig_lim = cv2.threshold(orig, 170, 255, cv2.THRESH_BINARY)
...
for i in range(len(img_lim)):
    for j in range(len(img_lim[i])):
        if (img_lim[i,j].any() == orig_lim[i,j].any()):
            eq_pixel += 1
        if (img_r_lim[i,j].any() == orig_lim[i,j].any()):
            eq_pixel2 += 1

print((eq_pixel/pixel_total)*100)
print((eq_pixel2/pixel_total)*100)
cv2.waitKey()
